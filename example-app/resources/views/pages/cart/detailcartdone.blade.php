        ...
        <div class="container">
            ...
            <div class="rounded" style="background-color: #99FFFF;">
                <table class="table">
                    ...
                    <tbody>
                        @if($getDataInfoCartDone != null)
                        @foreach($getDataInfoCartDone as $value)
                        <tr class="text-center">
                        <td><a href="{{route('getdataiteminfocartdone',$value->id)}}">{{$value->id}}</a></td>
                            <td><a href="{{route('getdataiteminfocartdone',$value->id)}}">{{$value->number_phone}}</a></td>
                            <td><a href="{{route('getdataiteminfocartdone',$value->id)}}">{{$value->address}}</a></td>
                            <td><a href="{{route('getdataiteminfocartdone',$value->id)}}">{{$value->updated_at}}</a></td>
                        </tr>
                        @endforeach
                        @endif
                    </tbody>
