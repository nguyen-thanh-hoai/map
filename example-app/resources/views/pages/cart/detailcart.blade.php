    ...
    <body>
      <div class="container">
        ...
        <div class="grid lg:grid-cols-4 grid-cols-1 gap-6 mt-10 ">
          @foreach($getDataCart as $value_cart)
          <div class="relative">
            ...
            <div class="flex gap-3 mt-3">
              <form action="{{route('deleteitemcart')}}" method="post">
                @csrf
                <input type="hidden" name="id" value="{{$value_cart->id}}">
                <button type="submit" class="p-2 rounded text-white" style="background-color: #FF0000;">Xóa</button>
              </form>
              <form action="{{route('getdataitemcartbyid')}}" method="post">
                @csrf
                <input type="hidden" name="id" value="{{$value_cart->id}}">
                <button type="submit" class="p-2 rounded text-white" style="background-color: #0000FF;">Chỉnh sửa</button>
              </form>
            </div>
          </div>
          @endforeach
        ...
        <div class="flex mt-10 items-center">
          ...
          <a class="mx-3 p-3 bg-dark text-white rounded" href="{{route('showCreateInfoCart')}}">Xác nhận đơn hàng</a>
        </div>
      </div>
